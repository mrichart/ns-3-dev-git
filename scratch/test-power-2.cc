  ...
  for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end (); ++i)
    {
      ...
      if ((t.sourceAddress=="10.1.1.3" && t.destinationAddress == "10.1.1.1"))
        {
		  NS_LOG_UNCOND(i->first  << " " << i->second.rxBytes * 8.0 / (i->second.timeLastRxPacket.GetSeconds() - i->second.timeFirstTxPacket.GetSeconds())/1024/1024
				  << " " << i->second.delaySum.GetSeconds() / i->second.rxPackets
				  << " " << i->second.jitterSum.GetSeconds() / (i->second.rxPackets - 1)
				  << " " << 1 - (stateCounter->GetBusyTime() / simuTime));
        }
      if ((t.sourceAddress=="10.1.1.4" && t.destinationAddress == "10.1.1.2"))
		{
		  NS_LOG_UNCOND(i->first  << " " << i->second.rxBytes * 8.0 / (i->second.timeLastRxPacket.GetSeconds() - i->second.timeFirstTxPacket.GetSeconds())/1024/1024
				  << " " << i->second.delaySum.GetSeconds() / i->second.rxPackets
				  << " " << i->second.jitterSum.GetSeconds() / (i->second.rxPackets - 1)
				  << " " << 1 - (stateCounter2->GetBusyTime() / simuTime));
		}
    }
