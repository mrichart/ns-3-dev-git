  ...
  for (uint32_t i = 0; i < nModes; i++)
    {
      ...
      txvector.SetMode(phy->McsToWifiMode(phy->GetMcs (i)));
  ...
  if (!station->m_initialized && GetNMcsSupported (station) > 1)
    {
      ...
      if (GetShortGuardInterval(station) && GetNumberOfReceiveAntennas(station)==2)
        m_nGroups = 4;
      else if ((GetShortGuardInterval(station) && GetNumberOfReceiveAntennas(station)==1) || (!GetShortGuardInterval(station) && GetNumberOfReceiveAntennas(station)==2))
        m_nGroups=2; ///< we have 2 groups one for LGI and one for SGI or one for 1 stream and one for 2 streams
      else
        m_nGroups=1;///< we only have 1 LGI group
      NS_LOG_DEBUG ("Number of Groups: " << (int) m_nGroups);
...
MinstrelHtWifiManager::GetGroupId(uint32_t rate, WifiRemoteStation *st, uint8_t txstreams)
{
  ...
  if (GetShortGuardInterval(station) && txstreams > 1)
   id =3; //SGI and Nss =2
  else if (!GetShortGuardInterval(station) && txstreams > 1)
   id = m_nGroups-(m_nGroups/2);//LGI and Nss =2 this is sometimes group 1 and sometimes group 2 depending on the number of groups
  else if (GetShortGuardInterval(station))
   id = 1;//rate / 8; //SGI and Nss = 1
  else
   id = 0; //LGI and Nss = 1
  return id;
}
      ...
      if (station->m_longRetry <  station->m_mcsTable[groupid].m_minstrelTable[rateid].adjustedRetryCount)
        {
            ...
            station->m_txrate = station->m_maxTpRate;  ///<  there's still a few retries left
        }
                                    ...
                                    uint32_t size)
{
  ...
  return WifiTxVector (GetPhy()->McsToWifiMode(GetMcsSupported (station, station->m_txrate)), GetDefaultTxPowerLevel (), st->m_slrc, st->m_state->m_shortGuardInterval,station->m_txstreams, st->m_state->m_tx, st->m_state->m_aggregation, st->m_state->m_stbc);
}
...
MinstrelHtWifiManager::DoGetRtsTxVector (WifiRemoteStation *st)
{
  ...
  return WifiTxVector (GetPhy()->McsToWifiMode(GetMcsSupported (station, 0)), GetDefaultTxPowerLevel (), st->m_ssrc, st->m_state->m_shortGuardInterval,station->m_txstreams, st->m_state->m_tx, st->m_state->m_aggregation, st->m_state->m_stbc);
}
  ...
  for (uint32_t i = 0; i < 8; i++)
    {
      ...
      NS_LOG_DEBUG (i << " " << (int) GetMcsSupported (station, GetTxRate(j,i)) <<
      ...
      for (uint32_t i = 0; i < 8; i++)
       {
         ...
         station->m_mcsTable[j].m_minstrelTable[i].perfectTxTime = GetCalcTxTime (GetPhy()->McsToWifiMode(GetMcsSupported (station, GetTxRate(j,i))));
