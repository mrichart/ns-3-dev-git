...
YansWifiPhy::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::YansWifiPhy")
    ...
    .AddAttribute ("Frequency", "The operating frequency.",
                   UintegerValue (2407),
                   MakeUintegerAccessor (&YansWifiPhy::GetFreq,
                                        &YansWifiPhy::SetFreq),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("Transmitters", "The number of transmitters.",
                   UintegerValue (1),
                   MakeUintegerAccessor (&YansWifiPhy::GetNumberOfTx,
                                        &YansWifiPhy::SetNumberOfTx),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("Recievers", "The number of recievers.",
                   UintegerValue (1),
                   MakeUintegerAccessor (&YansWifiPhy::GetNumberOfRx,
                                        &YansWifiPhy::SetNumberOfRx),
                   MakeUintegerChecker<uint32_t> ())
...
WifiMode txMode=txVector.GetMode();
  ...
  event = m_interference.Add (packet->GetSize (),
                              ...
                              rxDuration,
                              rxPowerW,
		          txVector);  //Added By Ghada for 11n we need it to calculate duration of HT training symbols
...
void
YansWifiPhy::SetFreq (uint32_t freq)
{
  m_freq = freq;
}

void
YansWifiPhy::SetNumberOfTx (uint32_t tx)
{
  m_tx = tx;
}
void
YansWifiPhy::SetNumberOfRx (uint32_t rx)
{
  m_rx = rx;
}
...
uint32_t
YansWifiPhy::GetFreq (void) const
{
  return m_freq;
}

uint32_t
YansWifiPhy::GetNumberOfTx (void) const
{
  return m_tx;
}
uint32_t
YansWifiPhy::GetNumberOfRx (void) const
{
  return m_rx;
}
...
{
  NS_LOG_FUNCTION (this);
  m_channelStartingFrequency = m_freq; // 2.4 (2407) or 5.000 (5e3) GHz
  m_deviceRateSet.push_back (WifiPhy::GetDsssRate1Mbps ());
  m_deviceRateSet.push_back (WifiPhy::GetDsssRate2Mbps ());
  m_deviceRateSet.push_back (WifiPhy::GetDsssRate5_5Mbps ());
  m_deviceRateSet.push_back (WifiPhy::GetErpOfdmRate6Mbps ());
