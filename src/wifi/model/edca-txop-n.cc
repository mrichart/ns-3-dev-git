    ...
    m_startTxop (Seconds (0))
{
  ...
  m_fastQueue = CreateObject<WifiMacQueue> ();
...
{
  NS_LOG_FUNCTION (this);
  return !m_tidQueueNew.empty() || !m_tidQueueOld.empty() || m_currentPacket != 0 || m_baManager->HasPackets ();
}
...
{
  return m_baManager->PeekNextPacketByTidAndAddress (m_queue, header,recipient,tid, timestamp);
}
  ...
  while (empty)
    {
          ...
          if (queueInfo->airtimeActive && m_stationManager->GetAirtimeDeficit (queueInfo->sta, queueInfo->tid) <= Seconds (0.0))
            {
              ...
              m_stationManager->IncreaseAirtimeDeficit (queueInfo->sta, queueInfo->tid, MicroSeconds(300));
      ...
      while (!found && !m_tidQueueOld.empty ())
        {
  ...
  if (m_currentPacket != 0)
    {
      ...
      //TODO debo mover la cola al final?
      m_currentPacket = 0;
    }
}
...
void
EdcaTxopN::Queue (Ptr<WifiMacQueue> queue, uint8_t tid, Mac48Address recipient)
{
  NS_LOG_FUNCTION (this << queue);
  //m_stationManager->PrepareForQueue (hdr.GetAddr1 (), &hdr, packet);
  //if (std::find_if (m_tidQueue.begin (), m_tidQueue.end (), TxQueueIsEqual) == m_tidQueue.end ())
  std::list<TxQueueInfo *>::iterator i = m_tidQueueNew.begin();
  bool found = false;
  while (!found && i != m_tidQueueNew.end()) {
    if ((*i)->queue == queue)
      {
        found = true;
      }
  ...
  i = m_tidQueueOld.begin();
  while (!found && i != m_tidQueueOld.end()) {
    if ((*i)->queue == queue)
      {
        found = true;
      }
...
EdcaTxopN::StartNext (void)
{
  ...
  WifiMacHeader hdr;
  Time tstamp;
  Ptr<const Packet> peekedPacket = 0;
...
EdcaTxopN::GotBlockAck (const CtrlBAckResponseHeader *blockAck, Mac48Address recipient, double rxSnr, WifiMode txMode, double dataSnr)
{
  ...
  m_baManager->NotifyGotBlockAck (m_queue, blockAck, recipient, rxSnr, txMode, dataSnr);
  if (!m_txOkCallback.IsNull ())
    {
      m_txOkCallback (m_currentHdr);
    }
...
EdcaTxopN::VerifyBlockAck (void)
{
  ...
  if (m_baManager->ExistsAgreementInState (recipient, tid, OriginatorBlockAckAgreement::INACTIVE))
    {
      m_baManager->SwitchToBlockAckIfNeeded (m_queue, recipient, tid, sequence);
    }
  ...
  if (m_currentHdr.IsQosData () && m_currentHdr.IsQosBlockAck ())
    {
      ...
      m_baManager->NotifyMpduTransmission (m_queue, m_currentHdr.GetAddr1 (), m_currentHdr.GetQosTid (),
...
EdcaTxopN::CompleteMpduTx (Ptr<const Packet> packet, WifiMacHeader hdr, Time tstamp)
{
  ...
  m_baManager->NotifyMpduTransmission (m_queue, hdr.GetAddr1 (), hdr.GetQosTid (),
