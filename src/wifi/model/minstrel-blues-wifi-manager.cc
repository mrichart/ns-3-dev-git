...
struct MinstrelBluesWifiRemoteStation : public WifiRemoteStation
{
  ...
  MinstrelBluesRate m_minstrelTable;  ///< minstrel table
  SampleRate m_sampleTable;  ///< sample table

  int m_bluesSampleCount;  ///< how many sample packets we have send so far
  int m_bluesRefCount;  ///< how many reference packets we have send so far
...
MinstrelBluesWifiManager::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::MinstrelBluesWifiManager")
    ...
    .AddAttribute ("LookAroundRate",
                   ...
                   DoubleValue (10),
                   MakeDoubleAccessor (&MinstrelBluesWifiManager::m_lookAroundRate),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("EWMA",
                   "EWMA level",
                   DoubleValue (75),
                   MakeDoubleAccessor (&MinstrelBluesWifiManager::m_ewmaLevel),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("SegmentSize",
                   "The largest allowable segment size packet",
                   DoubleValue (6000),
                   MakeDoubleAccessor (&MinstrelBluesWifiManager::m_segmentSize),
                   MakeDoubleChecker <double> ())
    .AddAttribute ("SampleColumn",
                   "The number of columns used for sampling",
                   DoubleValue (10),
                   MakeDoubleAccessor (&MinstrelBluesWifiManager::m_sampleCol),
                   MakeDoubleChecker <double> ())
    .AddAttribute ("PacketLength",
                   "The packet length used for calculating mode TxTime",
                   DoubleValue (1420),
                   MakeDoubleAccessor (&MinstrelBluesWifiManager::m_pktLen),
                   MakeDoubleChecker <double> ())
    .AddAttribute ("LookAroundSamplePower",
                   "The percentage of blues sampling",
                   DoubleValue (10),
                   MakeDoubleAccessor (&MinstrelBluesWifiManager::m_lookAroundSamplePower),
                   MakeDoubleChecker <double> ())
    .AddAttribute ("BluesUpdateStats",
                   "How many reference and sample packets are needed for updating stats",
                   UintegerValue (10),
                   MakeUintegerAccessor (&MinstrelBluesWifiManager::m_bluesUpdateStats),
                   MakeUintegerChecker <uint32_t> ())
    .AddAttribute ("IncrementLevel",
                   "How much levels to increase power each time",
                   UintegerValue (1),
                   MakeUintegerAccessor (&MinstrelBluesWifiManager::m_deltaInc),
                   MakeUintegerChecker <uint8_t> ())
    .AddAttribute ("DecrementLevel",
                   "How much levels to decrease power each time",
                   UintegerValue (1),
                   MakeUintegerAccessor (&MinstrelBluesWifiManager::m_deltaDec),
                   MakeUintegerChecker <uint8_t> ())
    .AddAttribute ("Delta",
                   "Power levels of separation between data and sample power",
                   UintegerValue (2),
                   MakeUintegerAccessor (&MinstrelBluesWifiManager::m_delta),
                   MakeUintegerChecker <uint8_t> ())
    .AddAttribute ("IncrementThreshold",
                   "The threshold between probabilities needed to increase power",
                   DoubleValue (0.1),
                   MakeDoubleAccessor (&MinstrelBluesWifiManager::m_thInc),
                   MakeDoubleChecker <double> ())
    .AddAttribute ("DecrementThreshold",
                   "The threshold between probabilities needed to decrease power",
                    DoubleValue (0.2),
                    MakeDoubleAccessor (&MinstrelBluesWifiManager::m_thDec),
                    MakeDoubleChecker <double> ())
    .AddAttribute ("UtilityWeight",
		   "The weight to use in the utility function",
		    DoubleValue (10),
		    MakeDoubleAccessor (&MinstrelBluesWifiManager::m_utilityWeight),
		    MakeDoubleChecker <double> ())
    .AddTraceSource ("DoGetDataTxVector",
                    "A TxVector is ask for a new data frame",
                    MakeTraceSourceAccessor (&MinstrelBluesWifiManager::m_getDataTxVector))
...
MinstrelBluesWifiManager::DoCreateStation (void) const
{
  ...
  station->m_bluesSampleCount = 100 / m_lookAroundSamplePower;
  ...
  if (!station->m_initialized && GetNSupported (station) > 1)
    {
      ...
      m_powerChange("init", station->m_txpower, station->m_state->m_address);
      station->m_txpower = m_nPower-1;
      m_rateChange(station->m_txrate, station->m_state->m_address);
      station->m_minstrelTable = MinstrelBluesRate (station->m_nsupported);
...
void
MinstrelBluesWifiManager::DoReportRxOk (WifiRemoteStation *st,
...
void
MinstrelBluesWifiManager::DoReportRtsOk (WifiRemoteStation *st, double ctsSnr, WifiMode ctsMode, double rtsSnr)
{
...
MinstrelBluesWifiManager::DoReportDataFailed (WifiRemoteStation *st)
{
  ...
  if (!station->m_isSampling && station->m_samplingBlues)
    {
      ...
      if (station->m_longRetry < station->m_minstrelTable[station->m_maxURate].adjustedRetryCount)
        {
      ...
      else if (station->m_longRetry <= (station->m_minstrelTable[station->m_maxURate].adjustedRetryCount +
                                         station->m_minstrelTable[station->m_maxURate2].adjustedRetryCount))
        {
          NS_LOG_DEBUG (" More retries left for the second maximum throughput rate.");
          station->m_txrate = station->m_maxURate2;
        }
      ...
      m_rateChange(station->m_txrate, station->m_state->m_address);
      m_powerChange("data", station->m_txpower, station->m_state->m_address);
    }
  ...
  /// for look-around rate, we're currently sampling random rates
  else if (station->m_isSampling)
    {
      ...
      m_rateChange(station->m_txrate, station->m_state->m_address);
      m_powerChange("ref", station->m_txpower, station->m_state->m_address);
    }
  //BLUES SAMPLING
  else
      {
	NS_LOG_DEBUG ("Look around rate is faster than the maximum throughput rate.");
	/// use random rate
	if (station->m_longRetry < station->m_minstrelTable[station->m_bluesSampleRate].adjustedRetryCount)
	  {
	    NS_LOG_DEBUG (" More retries left for the sampling rate.");
	    ...
	    m_rateChange(station->m_txrate, station->m_state->m_address);
	    m_powerChange("sample", station->m_txpower, station->m_state->m_address);
	  }

	/// use the best throughput rate
	else if (station->m_longRetry <= (station->m_minstrelTable[station->m_bluesSampleRate].adjustedRetryCount +
					  station->m_minstrelTable[station->m_maxTpRate].adjustedRetryCount))
	  {
	    NS_LOG_DEBUG (" More retries left for the maximum throughput rate.");
	    station->m_txrate = station->m_maxTpRate;
...
{
  uint32_t th = station->m_minstrelTable[rate].throughput;
  uint32_t bestTh = station->m_minstrelTable[station->m_maxTpRate].throughput;
  if (th != 0 && bestTh != 0)
    {
      double benefit = (th / bestTh)*100;
      uint8_t power = station->m_minstrelTable[rate].dataPower;
      uint8_t bestPower = station->m_minstrelTable[station->m_maxTpRate].dataPower;
      double cost = (power/th)*(bestTh/bestPower)*100;
      return m_utilityWeight*benefit-cost;
    }
  else
    {
      return 0;
  ...
  for (uint32_t i = 0; i < station->m_nsupported; i++)
    {
      ...
      if (station->m_minstrelTable[i].numSampleAttempt > m_bluesUpdateStats && station->m_minstrelTable[i].numRefAttempt > m_bluesUpdateStats)
        {
          /**
           * calculate the probability of success
           * assume probability scales from 0 to 18000
          ...
          if (station->m_minstrelTable[i].ewmaSampleProb < (station->m_minstrelTable[i].ewmaRefProb - m_thInc*18000))
            {
              station->m_minstrelTable[i].samplePower = Min((m_nPower-4),(station->m_minstrelTable[i].samplePower + m_deltaInc));
            }
          ...
          if (station->m_minstrelTable[i].ewmaRefProb > (18000 - m_thDec*18000))
            {
              station->m_minstrelTable[i].refPower = Max(3,(station->m_minstrelTable[i].refPower - m_deltaDec));
            }
      ...
      if (station->m_minstrelTable[i].numDataAttempt > m_bluesUpdateStats)
        {
          dataTempProb = (station->m_minstrelTable[i].numDataSuccess * 18000) / station->m_minstrelTable[i].numDataAttempt;
          station->m_minstrelTable[i].dataProb = dataTempProb;
  ...
  for (uint32_t i = 0; i < station->m_nsupported; i++)
    {
      ...
      station->m_minstrelTable[i].dataPower = m_nPower - 1;
      station->m_minstrelTable[i].refPower = m_nPower - 1;
      station->m_minstrelTable[i].samplePower = station->m_minstrelTable[i].refPower - 3;
...
MinstrelBluesWifiManager::PrintTable (MinstrelBluesWifiRemoteStation *station)
{
  ...
  for (uint32_t i = 0; i < station->m_nsupported; i++)
    {
      NS_LOG_UNCOND( std::setw(2) << i << std::setw(10) << station->m_minstrelTable[i].throughput << std::setw(10) << station->m_minstrelTable[i].ewmaProb << std::setw(10) << station->m_minstrelTable[i].prob
