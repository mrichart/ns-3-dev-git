...
private:
  ...
  std::map<Mac48Address, uint64_t> actualMode;
  ...
  for (uint32_t j = 0; j < stas.GetN (); j++)
    {
      ...
      actualPower[addr] = 17;
      actualMode[addr] = phy->GetMode(0).GetDataRate(20,false,1);
    }
  actualMode[Mac48Address ("ff:ff:ff:ff:ff:ff")] = phy->GetMode(0).GetDataRate(20,false,1);
  totalEnergy = 0;
  totalTime = 0;
  busyTime = 0;
  idleTime = 0;
...
void
NodeStatistics::RateCallback (std::string path, uint64_t rate, Mac48Address dest)
{
  actualMode[dest] =  rate / 1000000.0;
}
...
int main (int argc, char *argv[])
{
  ...
  Config::Connect ("/NodeList/1/DeviceList/*/$ns3::WifiNetDevice/RemoteStationManager/$" + apManager + "/RateChange",
                   MakeCallback (&NodeStatistics::RateCallback, &statisticsAp1));
  ...
  Config::Connect ("/NodeList/[0-1]/DeviceList/*/$ns3::WifiNetDevice/RemoteStationManager/$" + apManager + "/RateChange",
